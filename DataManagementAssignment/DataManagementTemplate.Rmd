...
```{r}
nesarc.sub <- nesarc.sub %>% 
  rename(Depression = MAJORDEPLIFE, TobacoDependence = TAB12MDX, SmokingFreq = S3AQ3B1, DailyCigsSmoked = S3AQ3C1, Ethnicity = ETHRACE2A, Sex = SEX, Age = AGE)
head(nesarc.sub)
```

### Creating Tables

Basic tables can be created using the functions `table` or `xtabs`. Frequency tables are created for the variables `TobacoDependence`,  `CigsSmokedFac`, and `SmokingFreq`.

```{r}
T1 <- xtabs(~TobacoDependence, data = nesarc.sub)
T2 <- xtabs(~CigsSmokedFac, data = nesarc.sub)
T3 <- xtabs(~SmokingFreq, data = nesarc.sub)
table(nesarc.sub$TobacoDependence)
T1
T2
T3
```
...
```{r}
library(ggplot2)
g1 <- ggplot(data = nesarc.sub, aes(x = TobacoDependence)) + 
  geom_bar() 
g1
