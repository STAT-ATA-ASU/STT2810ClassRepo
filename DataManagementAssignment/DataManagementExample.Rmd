...
Three categorical variables relevant to *student health* are `H1SU1-H1SU3` from the `AddHealth` data set.  The `AddHealth` data set is loaded from the `PDS` package.  Using the `dplyr` package, a new data frame `NDF` containing only `H1SU1-H1SU3` and renaming `H1SU1-H1SU3` to `ConsideredSuicide`, `TimesAttemptedSuicide`, `TreatedByDoctor` is created.
...
Question `TreatedByDoctor` asks of the `r NAS` that attempted suicide if they had to be treated by either a nurse or doctor.  The response of 7 should be recoded to `NA`.
...
freq(NDF$TreatedByDoctor, main = "Did you require treatment by either a nurse of doctor?")
