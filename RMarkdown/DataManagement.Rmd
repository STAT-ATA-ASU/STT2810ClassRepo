...
```{r label = "READIN", comment = NA, cache = TRUE}
...
```{r label = "ggSmoke", message = FALSE, fig.width = 10, fig.wdith = 5, cache = TRUE}
library(ggplot2)
...
```{r label = "ggSmoke2", message = FALSE, fig.width = 10, fig.wdith = 5, cache = TRUE}
...
```{r label = "CollapseHS", comment = NA, cache = TRUE}
...
```{r label = "CUT1", comment = NA, cache = TRUE}
...
```{r label = "CUT2", comment = NA, cache = TRUE}
...
```{r label = "Agg1", comment = NA, cache = TRUE}
...
```{r label = "PANIC", comment = NA, cache = TRUE}
...
sdfm <- as.matrix(sdf, ncol = 19)
myfix <- function(x){ifelse(x %in% c(2, 9), 0, ifelse(x == 1, 1, NA))}
sdfm <- apply(sdfm, 2, myfix)
sdfm <- data.frame(sdfm)
nsdf <- cbind(sdfm, NDS = apply(sdfm, 1, sum, na.rm = FALSE))
nsdf[11000:11005, 14:20]
xtabs(~NDS, data = nsdf)
```

To create a variable `NumDepSym` that records the number of depression symptoms among the variables `S4AQ4A1` - `S4AQ4A19`, create numeric variables from the factors `S4AQ4A1` - `S4AQ4A19` and store the results in `S4AQ4A1num` - `S4AQ4A19num`.  Next, recode the levels of `No` (2) and `Unknown` (3) to 0s for variables `S4AQ4A1num` - `S4AQ4A19num`.  The code is rather lengthy and other approaches such as the first one are encouraged.  The template given in the PDS IBook link will not work with this data because the variables are factors not numeric vectors.  You can not add factors!

```{r label = "CompositeFactor", comment = NA, cache = TRUE}
summary(NESARC$S4AQ4A1)
...
```{r label = "SUB1", comment = NA, cache = TRUE}
NESARCsub1 <- NESARC %>%
...
```{r label = "SUB2", comment = NA, cache = TRUE}
...
```{r label = "SUB3", comment = NA, cache = TRUE}
...
```{r label = "ggNV", comment = NA, fig.align='center', cache = TRUE}
...
```{r label = "WtHist", comment = NA, fig.align = "center", cache = TRUE}
...
```{r label = "LastTwo", comment = NA, fig.align = 'center', cache = TRUE}
...
```{r label = "ANOVAexp", comment = NA, fig.align = 'center', cache = TRUE}
...
```{r label = "qqP1",comment = NA, fig.align='center', message=FALSE, cache = TRUE}
library(car)
qqPlot(my.aov)
```
...
```{r label = "REGexp", comment = NA, cache = TRUE}
...
```{r label = "qqP2", comment = NA, fig.align = 'center', message = FALSE, cache = TRUE}
qqPlot(my.lm)
qqPlot(my.lm2)
```
