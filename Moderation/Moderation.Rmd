...
```{r}
library(PASWR2)
T1 <- xtabs(~pclass + survived, data = TITANIC3)
T1
chisq.test(T1)
by(TITANIC3, TITANIC3$sex, function(x)list(chisq.test(x$pclass, x$survived),
                                               chisq.test(x$pclass, x$survived)$residuals))
# Or
T1a <- xtabs(~pclass + survived, data = subset(TITANIC3, subset = sex == "female"))
T1a
chisq.test(T1a)
chisq.test(T1a)$res
T1b <- xtabs(~pclass + survived, data = subset(TITANIC3, subset = sex == "male"))
T1b
chisq.test(T1b)
chisq.test(T1b)$res
library(vcd)
