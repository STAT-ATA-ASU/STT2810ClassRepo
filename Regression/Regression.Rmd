...
$$\frac{e^{\beta_0}}{1 + e^{\beta_0}}$$ for $p(X=0)$, repsectively, one can show that
$$\text{OR} = e^{\beta_1}.$$
...
Using the `mlog.mod` model, the response variable $Y$  is `default`, which denotes whether a credit card holder has defaulted (Yes/1) or not defaulted (No/0) on his/her current payment. The independent variable $X$  is `student`, which denotes whether the individual is a student (Yes/1) or is not a student (No/1).  An odds ratio in this scenario of 2 is interpreted to mean that the odds of default among students is two times greater than the odds of default among non-students.  An estimate of the odds ratio is
